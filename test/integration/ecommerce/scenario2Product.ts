...
export function runScenario(ctx: ScenarioContext): void {
  ...
  describe('Phase A: Developer', () => {
    ...
    it('A1: creates feature/product-catalog branch', async function () {
      this.timeout(180000);
  ...
  describe('Phase B: PR workflow', function () {
    ...
    it('B1: creates PR', async () => {
      prNumber = await createPR(ctx, 'Add product catalog with inventory', BRANCH);
    ...
    it('B3: PR comment contains schema diff', async () => {
      ...
      const schemaDiffComment = comments.find(c => c.includes('product'));
