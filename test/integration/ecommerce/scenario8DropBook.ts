...
export function runScenario(ctx: ScenarioContext): void {
  ...
  describe('Phase A: Developer', () => {
    ...
    it('A1: creates feature/remove-book branch', async function () {
      this.timeout(300000);
  ...
  describe('Phase B: PR workflow', function () {
    ...
    it('B1: creates PR', async () => {
      prNumber = await createPR(ctx, 'Remove book entity \u2014 discontinued product line', BRANCH);
    ...
    it('B3: PR comment contains schema diff', async () => {
      ...
      const schemaDiffComment = comments.find(c =>
        c.includes('book') &&
        (c.includes('DROP') || c.includes('REMOVED'))
      );
